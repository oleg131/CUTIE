...
def cal_accuracy(data_loader, grid_table, gt_classes, model_output_val, label_mapids, bbox_mapids):
    ...
    for b in range(grid_table.shape[0]):
        ...
        logits = model_output_val[b, :, :, :].reshape(
            [-1, data_loader.num_classes])
        label_mapid = label_mapids[b]
        bbox_mapid = bbox_mapids[b]
        rows, cols = grid_table.shape[1:3]
        bbox_id = np.array([row*cols+col for row in range(rows)
        ...
        for c in range(1, data_loader.num_classes):
            ...
            logits_indexes = np.where(
                logits_array_selected[:, c] > c_threshold)[0]

            labels_words = list(
                data_loader.index_to_word[i] for i in data_selected[labels_indexes])
            logits_words = list(
                data_loader.index_to_word[i] for i in data_selected[logits_indexes])

            # GT bbox_ids related to the type of class
            label_bbox_ids = label_mapid[c]
            logit_bbox_ids = [bbox_mapid[bbox]
                              for bbox in bbox_id_selected[logits_indexes] if bbox in bbox_mapid]
            ...
            if set(label_bbox_ids) == set(logit_bbox_ids):  # decide as correct when all ids match
                ...
                num_correct_soft += 1
            # correct when gt is subset of gt
            elif set(label_bbox_ids).issubset(set(logit_bbox_ids)):
                num_correct_soft += 1
            try:  # calculate prevalence with decimal precision
                num_correct += np.shape(np.intersect1d(labels_indexes, logits_indexes))[
                    0] / np.shape(labels_indexes)[0]
            ...
            if b == 0:
                ...
                # ground truth label
                res += ' '.join(data_loader.index_to_word[i]
                                for i in data_selected[labels_indexes])
                res += '" | "'
                res += ' '.join(data_loader.index_to_word[i]
                ...
                if not np.array_equal(labels_indexes, logits_indexes):
                    ...
                    fault_logits_indexes = np.setdiff1d(
                        logits_indexes, labels_indexes)
...
def cal_save_results(data_loader, grid_table, gt_classes, model_output_val, label_mapids, bbox_mapids, file_names, save_prefix):
    ...
    for b in range(grid_table.shape[0]):
        ...
        for c in range(1, data_loader.num_classes):
            ...
            # ground truth label
            gt = str(
                ' '.join(data_loader.index_to_word[i] for i in data_selected[labels_indexes]))
            predict = str(
                ' '.join(data_loader.index_to_word[i] for i in data_selected[logits_indexes]))

            # write results to csv
            fieldnames = ['TaskID', 'GT', 'Predicted']

            csv_filename = 'data/results/' + save_prefix + \
                '_' + data_loader.classes[c] + '.csv'
            writer = csv.DictWriter(
                open(csv_filename, 'a'), fieldnames=fieldnames)
            row = {'TaskID': filename, 'GT': gt, 'Predicted': predict}
            writer.writerow(row)

            csv_diff_filename = 'data/results/' + save_prefix + \
                '_Diff_' + data_loader.classes[c] + '.csv'
            if gt != predict:
                writer = csv.DictWriter(
                    open(csv_diff_filename, 'a'), fieldnames=fieldnames)
                row = {'TaskID': filename, 'GT': gt, 'Predicted': predict}
                writer.writerow(row)
...
def vis_bbox(data_loader, file_prefix, grid_table, gt_classes, model_output_val, file_name, bboxes, shape):
    ...
    if img is not None:
        ...
        gt_color.extend([[0, 0, 0]] * 100)
        inf_color.extend([[0, 0, 0]] * 100)
        ...
        for i in range(len(data_input_flat)):
            ...
            if data_input_flat[i] and labels[i]:
                gt_id = labels[i]
                cv2.rectangle(overlay_box, (x, y),
                              (x+w, y+h), gt_color[gt_id], -1)
        ...
        for i in range(1, len(data_loader.classes)):
            ...
            cv2.putText(
                img, data_loader.classes[i], (x+w, y+h), font, 0.8, ft_color)
...
def cal_accuracy_table(data_loader, grid_table, gt_classes, model_output_val, label_mapids, bbox_mapids):
    ...
    for b in range(grid_table.shape[0]):
        ...
        labels = gt_classes[b, :, :]
        logits = model_output_val[b, :, :, :].reshape(
            [rows, cols, data_loader.num_classes])
        label_mapid = label_mapids[b]
        bbox_mapid = bbox_mapids[b]
        bbox_id = np.array([row*cols+col for row in range(rows)
                            for col in range(cols)])
...
def vis_table(data_loader, file_prefix, grid_table, gt_classes, model_output_val, file_name, bboxes, shape):
    ...
    if img is not None:
        ...
        cv2.rectangle(overlay_box, (gt_x, gt_y),
                      (gt_r, gt_b), [180, 180, 215], -1)
        cv2.rectangle(overlay_line, (inf_x+bbox_pad, inf_y+bbox_pad),
                      (inf_r+bbox_pad, inf_b+bbox_pad), [0, 115, 255], max_len//768*2)
