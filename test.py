...
parser.add_argument('--doc_path', type=str,
                    default='data/SROIE')  # modify this
parser.add_argument('--save_prefix', type=str, default='SROIE',
                    help='prefix for load ckpt model')  # modify this
# leave empty if no test data provided
parser.add_argument('--test_path', type=str, default='')

# augment data row/col in each batch
parser.add_argument('--fill_bbox', type=bool, default=False)

parser.add_argument('--e_ckpt_path', type=str,
                    default='../graph/CUTIE/graph/')  # modify this
parser.add_argument('--ckpt_file', type=str,
                    default='CUTIE_atrousSPP_d20000c5(r80c80)_iter_29201.ckpt')
...
parser.add_argument('--load_dict_from_path', type=str,
                    default='dict/SROIEnc')  # 40000 or table or 20000TC
...
if __name__ == '__main__':
    ...
    data_loader = DataLoader(params, update_dict=False,
                             load_dictionary=True, data_split=False)
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            ckpt_path = os.path.join(
                params.e_ckpt_path, params.save_prefix, params.ckpt_file)
...
#                                                        data['file_name'][0], params.save_prefix)
            ...
            if not params.is_table:
                vis_bbox(data_loader, params.doc_path, np.array(data['grid_table'])[0],
                         np.array(data['gt_classes'])[0], np.array(
                             model_output_val)[0], file_name,
