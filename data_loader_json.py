...
class DataLoader():
    ...
    def __init__(self, params, update_dict=True, load_dictionary=False, data_split=0.75):
        ...
        # ensures the size (rows/cols) of grid table compat with the network
        self.encoding_factor = 1
        ...
        if self.tokenize:
            self.tokenizer = tokenization.FullTokenizer(
                'dict/vocab.txt', do_lower_case=not self.text_case)

        self.rows = self.encoding_factor  # to be updated
        self.cols = self.encoding_factor  # to be updated
        # segment grid into two parts if grid is larger than cols_target
        self.segment_grid = params.segment_grid if hasattr(
            params, 'segment_grid') else False
        self.augment_strategy = params.augment_strategy if hasattr(
            params, 'augment_strategy') else 1
        self.pm_strategy = params.positional_mapping_strategy if hasattr(
            params, 'positional_mapping_strategy') else 2
        self.rows_segment = params.rows_segment if hasattr(
            params, 'rows_segment') else 72
        self.cols_segment = params.cols_segment if hasattr(
            params, 'cols_segment') else 72
        self.rows_target = params.rows_target if hasattr(
            params, 'rows_target') else 64
        self.cols_target = params.cols_target if hasattr(
            params, 'cols_target') else 64
        # handle OOM, must be multiple of self.encoding_factor
        self.rows_ulimit = params.rows_ulimit if hasattr(
            params, 'rows_ulimit') else 80
        # handle OOM, must be multiple of self.encoding_factor
        self.cols_ulimit = params.cols_ulimit if hasattr(
            params, 'cols_ulimit') else 80

        # fill bbox with labels or use one single lable for the entire bbox
        self.fill_bbox = params.fill_bbox if hasattr(
            params, 'fill_bbox') else False

        self.data_augmentation_dropout = params.data_augmentation_dropout if hasattr(
            params, 'data_augmentation_dropout') else False  # TBD: randomly dropout rows/cols
        self.data_augmentation_extra = params.data_augmentation_extra if hasattr(
            params, 'data_augmentation_extra') else False  # randomly expand rows/cols
        self.da_extra_rows = params.data_augmentation_extra_rows if hasattr(
            params, 'data_augmentation_extra_rows') else 0  # randomly expand rows/cols
        self.da_extra_cols = params.data_augmentation_extra_cols if hasattr(
            params, 'data_augmentation_extra_cols') else 0  # randomly expand rows/cols

        # 0> parameters to be tuned
        # load dictionary from file rather than start from empty
        ...
        if self.load_dictionary:
            self.dictionary = np.load(
                self.dict_path + '_dictionary.npy').item()
            self.word_to_index = np.load(
                self.dict_path + '_word_to_index.npy').item()
            self.index_to_word = np.load(
                self.dict_path + '_index_to_word.npy').item()
        else:
            # word/counts. to be updated in self.load_data() and self._update_docs_dictionary()
            self.dictionary = {'[PAD]': 0, '[UNK]': 0}
            self.word_to_index = {}
            self.index_to_word = {}

        # split data to training/validation, 0 for all for validation
        self.data_split = data_split
        # 0 to consider key and value as two different class, 1 the same class, 2 only value considered
        self.data_mode = 2
        self.remove_lowfreq_words = False  # remove low frequency words when set as True

        self.num_classes = len(self.classes)
        self.batch_size = params.batch_size if hasattr(
            params, 'batch_size') else 1
        ...
        self.training_docs, self.training_labels = self.load_data(
            self.training_doc_files, update_dict=update_dict)  # TBD: optimize the update dict flag
        ...
        # remove low frequency words and add it under the <unknown> key
        self._update_docs_dictionary(
            self.training_docs, 3, self.remove_lowfreq_words)
        ...
        if self.random:
            selected_training_index = random.sample(
                data_to_be_fetched, num_training)
        selected_validation_index = list(
            set(data_to_be_fetched).difference(set(selected_training_index)))
        self.validation_docs = [self.training_docs[x]
                                for x in selected_validation_index]
        self.training_docs = [self.training_docs[x]
                              for x in selected_training_index]
        self.validation_labels = self.training_labels
        print('\n\nDATASET: %d vocabularies, %d target classes' %
              (len(self.dictionary), len(self.classes)))
        print('DATASET: %d for training, %d for validation' %
              (len(self.training_docs), len(self.validation_docs)))

        # 1.2> load test files
        self.test_doc_files = self._get_filenames(
            params.test_path) if hasattr(params, 'test_path') else []
        self.test_docs, self.test_labels = self.load_data(
            self.test_doc_files, update_dict=update_dict)  # TBD: optimize the update dict flag
        print('DATASET: %d for test from %s \n' % (len(self.test_docs),
                                                   params.test_path if hasattr(params, 'test_path') else '_'))
        ...
        if len(self.training_docs) > 0:  # adapt grid table size to all training dataset docs
            self.rows, self.cols, _, _ = self._cal_rows_cols(
                self.training_docs)
        ...
        self.training_data_tobe_fetched = [
            i for i in range(len(self.training_docs))]
        self.validation_data_tobe_fetched = [
            i for i in range(len(self.validation_docs))]
    ...
    def _updae_word_to_index(self):
        if self.load_dictionary:
            ...
            self.word_to_index = dict(
                list(zip(self.dictionary.keys(), list(range(self.num_words)))))
            self.index_to_word = dict(
                list(zip(list(range(self.num_words)), self.dictionary.keys())))
    ...
    def next_batch(self):
        ...
        while True:
            ...
            selected_index = random.sample(
                self.training_data_tobe_fetched, batch_size)
            self.training_data_tobe_fetched = list(
                set(self.training_data_tobe_fetched).difference(set(selected_index)))

            training_docs = [self.training_docs[x] for x in selected_index]

            # data augmentation in each batch if self.data_augmentation==True
            rows, cols, pre_rows, pre_cols = self._cal_rows_cols(
                training_docs, extra_augmentation=self.data_augmentation_extra, dropout=self.data_augmentation_dropout)
            ...
            grid_table, gt_classes, bboxes, label_mapids, bbox_mapids, file_names, updated_cols, ps_indices_x, ps_indices_y = \
                self._positional_mapping(
                    training_docs, self.training_labels, rows, cols)
            if updated_cols > cols:
                ...
                grid_table, gt_classes, bboxes, label_mapids, bbox_mapids, file_names, _, ps_indices_x, ps_indices_y = \
                    self._positional_mapping(
                        training_docs, self.training_labels, rows, updated_cols, update_col=False)

            # load image and generate corresponding @ps_1dindices
            images, ps_1d_indices = [], []
            if self.use_cutie2:
                images, ps_1d_indices = self._positional_sampling(
                    self.doc_path, file_names, ps_indices_x, ps_indices_y, updated_cols)
    ...
    def fetch_validation_data(self):
        ...
        while True:
            if len(self.validation_data_tobe_fetched) == 0:
                self.validation_data_tobe_fetched = [
                    i for i in range(len(self.validation_docs))]
            selected_index = random.sample(
                self.validation_data_tobe_fetched, 1)
            ...
            real_rows, real_cols, _, _ = self._cal_rows_cols(
                validation_docs, extra_augmentation=False)
            rows = max(self.rows_target, real_rows)
            cols = max(self.rows_target, real_cols)

            grid_table, gt_classes, bboxes, label_mapids, bbox_mapids, file_names, updated_cols, ps_indices_x, ps_indices_y = \
                self._positional_mapping(
                    validation_docs, self.validation_labels, rows, cols)
            if updated_cols > cols:
                print('Validation grid EXPAND size: ({},{}) from ({},{})'
                      .format(rows, updated_cols, rows, cols))
                grid_table, gt_classes, bboxes, label_mapids, bbox_mapids, file_names, _, ps_indices_x, ps_indices_y = \
                    self._positional_mapping(
                        validation_docs, self.validation_labels, rows, updated_cols, update_col=False)
        ...
        def build_gt_pyramid(self, gt_classes):
            ...
            for gt_class in gt_classes:
                pad_v = np.pad(gt_class, ((same_padding_top, same_padding_bottom),
                                          (0, 0)), 'constant', constant_values=((0, 0), (0, 0)))
                pad_h = np.pad(gt_class, ((0, 0), (same_padding_left, same_padding_right)),
    ...
    def fetch_test_data(self):
        ...
        while True:
            if len(self.test_data_tobe_fetched) == 0:
                self.test_data_tobe_fetched = [
                    i for i in range(len(self.test_docs))]
                return None

            selected_index = self.test_data_tobe_fetched[0]
            self.test_data_tobe_fetched = list(
                set(self.test_data_tobe_fetched).difference(set([selected_index])))

            test_docs = [self.test_docs[selected_index]]

            real_rows, real_cols, _, _ = self._cal_rows_cols(
                test_docs, extra_augmentation=False)
            # small shaped documents have better performance with shape 64
            rows = max(self.rows_target, real_rows)
            # large shaped docuemnts have better performance with shape 80
            cols = max(self.cols_target, real_cols)

            grid_table, gt_classes, bboxes, label_mapids, bbox_mapids, file_names, updated_cols, ps_indices_x, ps_indices_y = \
                self._positional_mapping(
                    test_docs, self.test_labels, rows, cols)
            if updated_cols > cols:
                print('Test grid EXPAND size: ({},{}) from ({},{})'
                      .format(rows, updated_cols, rows, cols))
                grid_table, gt_classes, bboxes, label_mapids, bbox_mapids, file_names, _, ps_indices_x, ps_indices_y = \
                    self._positional_mapping(
                        test_docs, self.test_labels, rows, updated_cols, update_col=False)

            # load image and generate corresponding @ps_1dindices
            images, ps_1d_indices = [], []
            if self.use_cutie2:
                images, ps_1d_indices = self._positional_sampling(
                    self.doc_test_path, file_names, ps_indices_x, ps_indices_y, updated_cols)
                if len(images) == batch_size:
                    break
            else:
                break
    ...
    def _form_label_matrix(self, gt_classes, target_h, target_w):
        ...
        for i in range(target_h):
            for j in range(target_w):
                ...
                roi = [rate_h*j, rate_v*i, rate_h*(j+1), rate_v*(i+1)]

                dst_classes[i][j] = has_entity_with_augmentation(
                    entity_ranges, roi, False)

                mask = gt_classes[roi[1]:roi[3], roi[0]:roi[2]]
                dst_masks[i][j] = mask if dst_classes[i][j] else np.zeros(
                    np.shape(mask))
    ...
    def data_shape_statistic(self):
        ...
        tss, tss_r, tss_c = shape_statistic(
            self.training_docs)  # training shape static
        ...
        if self.data_laundry:
            print("\nRemoving grids with shape larger than ({},{}).".format(
                self.rows_ulimit, self.cols_ulimit))
    ...
    def _positional_mapping(self, docs, labels, rows, cols):
        ...
        for doc in docs:
            ...
            # one word in one or many positions in a bbox is mapped in bbox_mapid
            bbox_id, bbox_mapid = 0, {}
            # each class is connected to several bboxes (words)
            label_mapid = [[] for _ in range(self.num_classes)]
            ...
            for item in doc:
                ...
                entity_id, class_id = self._dress_class(
                    file_name, word_id, labels)
...
#                             bbox[y][x] = [x_left, y_top, x_right-x_left, y_bottom-y_top]
                ...
                v_c = (y_top - top + (y_bottom-y_top)/2) / (bottom-top)
                h_c = (x_left - left + (x_right-x_left)/2) / \
                    (right-left)  # h_c is used for sorting items
                row = int(rows * v_c)
                col = int(cols * h_c)
                items.append([row, col, [box_y, box_x], [v_c, h_c], file_name, dict_id, class_id,
                              entity_id, bbox_id, [x_left, y_top, x_right-x_left, y_bottom-y_top]])

            # sort according to row > h_c > bbox_id
            items.sort(key=lambda x: (x[0], x[3], x[5]))
            for item in items:
                row, col, [box_y, box_x], [
                    v_c, h_c], file_name, dict_id, class_id, entity_id, bbox_id, box = item
        ...
        for file in data_files:
            with open(file, encoding='utf-8') as f:
                ...
                data = self._collect_data(
                    file_id, data['text_boxes'], update_dict)
                for i in data:
    ...
    def _cal_rows_cols(self, docs, extra_augmentation=False, dropout=False):
        ...
        # (max_row//self.encoding_factor+1) * self.encoding_factor
        pre_rows = self._fit_shape(max_row)
        # (max_col//self.encoding_factor+1) * self.encoding_factor
        pre_cols = self._fit_shape(max_col)

        rows, cols = 0, 0
        if extra_augmentation:
            # abs(random.gauss(0, u))
            pad_row = int(random.gauss(
                0, self.da_extra_rows*self.encoding_factor))
            # random.randint(0, u)
            pad_col = int(random.gauss(
                0, self.da_extra_cols*self.encoding_factor))

            if self.augment_strategy == 1:  # strategy 1: augment data by increasing grid shape sizes
                pad_row = abs(pad_row)
                pad_col = abs(pad_col)
                # apply upper boundary to avoid OOM
                rows = self._fit_shape(max_row+pad_row)
                # apply upper boundary to avoid OOM
                cols = self._fit_shape(max_col+pad_col)
            # strategy 2: augment by increasing or decreasing the target gird shape size
            elif self.augment_strategy == 2 or self.augment_strategy == 3:
                # protect grid shape
                rows = self._fit_shape(max(self.rows_target+pad_row, max_row))
                # protect grid shape
                cols = self._fit_shape(max(self.cols_target+pad_col, max_col))
            else:
                raise Exception('unknown augment strategy')
            # apply upper boundary to avoid OOM
            rows = min(rows, self.rows_ulimit)
            # apply upper boundary to avoid OOM
            cols = min(cols, self.cols_ulimit)
        else:
            rows = pre_rows
            cols = pre_cols
    ...
    def _collect_data(self, file_name, content, update_dict):
        ...
        for line in content:
            ...
            for i, dressed_text in enumerate(dressed_texts):
                ...
                content_dressed.append([file_name, dressed_text, word_id, [
                                       new_left, y_top, new_right, y_bottom]])

        # initial calculation of maximum number of words in rows/cols in terms of image size
        # number of words in each row
        num_words_row = [0 for _ in range(bottom)]
        # number of words in each column
        num_words_col = [0 for _ in range(right)]
        ...
        while not DONE:
            ...
            for line in content_dressed:
                _, _, _, [x_left, y_top, x_right, y_bottom] = line
                row = int(max_rows * (y_top - top +
                                      (y_bottom-y_top)/2) / (bottom-top))
                col = int(max_cols * (x_left - left +
                                      (x_right-x_left)/2) / (right-left))
        ...
        if self.segment_grid and max_cols > self.cols_segment:
            ...
            # counter for number of words in a specific row
            cnt = defaultdict(int)
            # update max_cols if larger than self.cols_segment
            ...
            for i, line in enumerate(content_dressed):
                file_name, dressed_text, word_id, [
                    x_left, y_top, x_right, y_bottom] = line
    ...
    def _collect_label(self, file_id, content):
        ...
        for line in content:
            ...
            if cls in self.classes:
                ...
                label_dressed[file_id][cls].append(
                    {'key_id': [], 'value_id': [], 'key_text': '', 'value_text': ''})
                label_dressed[file_id][cls][-1]['key_id'] = line.get(
                    'key_id', [])
                # value_id
                label_dressed[file_id][cls][-1]['value_id'] = line['value_id']
                label_dressed[file_id][cls][-1]['key_text'] = line.get(
                    'key_text', [])
                # value_text
                    ...
                    for key, values in cls_label.items():
                        if (key == 'key_id' or key == 'value_id') and word_id in values:
                            if key == 'key_id':
                                if self.data_mode == 0:
                                    # odd
                                    return idx, self.classes.index(cls) * 2 - 1
                                ...
                                if self.data_mode == 0:
                                    # even
                                    return idx, self.classes.index(cls) * 2
    ...
    def grid_visualization(self, file_name, grid, label):
        ...
        cv2.putText(palette, file_name+"({},{})".format(height, width),
                    (grid_box_h, grid_box_w), font, 0.6, [255, 0, 0])
        for h in range(height):
            cv2.line(palette, (0, h*grid_box_h),
                     (width*grid_box_w, h*grid_box_h), (100, 100, 100))
            for w in range(width):
                if grid[h, w]:
                    org = (int((w+1)*grid_box_w*0.7),
                           int((h+1)*grid_box_h*0.9))
                    color = gt_color[label[h, w]]
                    cv2.putText(
                        palette, self.index_to_word[grid[h, w]], org, font, 0.4, color)
