...
# data
# True to read image from doc_path
parser.add_argument('--use_cutie2', type=bool, default=False)
parser.add_argument('--doc_path', type=str, default='data/SROIE')
parser.add_argument('--save_prefix', type=str, default='SROIE',
                    help='prefix for ckpt')  # TBD: save log/models with prefix
# leave empty if no test data provided
...
parser.add_argument('--embedding_file', type=str,
                    default='../graph/bert/multi_cased_L-12_H-768_A-12/bert_model.ckpt')
parser.add_argument('--ckpt_path', type=str, default='../graph/CUTIE/graph/')
parser.add_argument('--ckpt_file', type=str,
                    default='meals/CUTIE_highresolution_8x_d20000c9(r80c80)_iter_40000.ckpt')

# dict
parser.add_argument('--load_dict', type=bool, default=True,
                    help='True to work based on an existing dict')
parser.add_argument('--load_dict_from_path', type=str,
                    default='dict/SROIE')  # 40000 or 20000TC or table
...
parser.add_argument('--segment_grid', type=bool, default=False)
parser.add_argument('--rows_segment', type=int, default=72)
parser.add_argument('--cols_segment', type=int, default=72)
# 1 for increasing grid shape size, 2 for gaussian around target shape
parser.add_argument('--augment_strategy', type=int, default=1)
...
# used when data augmentation is true
parser.add_argument('--rows_ulimit', type=int, default=80)
parser.add_argument('--cols_ulimit', type=int, default=80)
# fill bbox with dict_id / label_id
parser.add_argument('--fill_bbox', type=bool, default=False)

parser.add_argument('--data_augmentation_extra', type=bool,
                    default=True)  # randomly expand rows/cols
...
# loss optimization
parser.add_argument('--hard_negative_ratio', type=int,
                    help='the ratio between negative and positive losses', default=3)
# 1 to use GHM, 0 to not use
parser.add_argument('--use_ghm', type=int, default=0)
...
# model
# not used for bert embedding which has 768 as default
...
def calc_ghm_weights(logits, labels):
    ...
    # equation for logits from the sigmoid activation
    grad = abs(logits_flat - labels_flat)
...
if __name__ == '__main__':
    ...
    data_loader = DataLoader(params, update_dict=params.update_dict,
                             load_dictionary=params.load_dict, data_split=0.75)
    ...
    summary_path = os.path.join(
        params.log_path, params.save_prefix, network.name)
    summary_writer = tf.summary.FileWriter(
        summary_path, tf.get_default_graph(), flush_secs=10)
    ...
    with tf.Session(config=config) as sess:
        ...
        if params.restore_ckpt:
            if params.restore_bertembedding_only:
                ...
                try:
                    load_variable = {
                        "bert/embeddings/word_embeddings": network.embedding_table}
                ...
                try:
                    ckpt_path = os.path.join(
                        params.ckpt_path, params.ckpt_file)
                    ckpt = tf.train.get_checkpoint_state(ckpt_path)
                    print('Restoring from {}...'.format(ckpt_path))
                    ckpt_saver.restore(sess, ckpt_path)
                    print('Restored from {}'.format(ckpt_path))
                    ...
                    raise Exception(
                        'Check your pretrained {:s}'.format(ckpt_path))
        ...
        for iter in range(iter_start, params.iterations+1):
            ...
            feeds = [network.data_grid, network.gt_classes,
                     network.data_image, network.ps_1d_indices, network.ghm_weights]
            fetches = [model_loss, regularization_loss, total_loss,
                       summary_op, train_dummy, model_logits, model_output]
            ...
            (model_logit_val, model_output_val) = sess.partial_run(
                h, fetches, feed_dict)

            # one step training
            ghm_weights = np.ones(np.shape(model_logit_val))
            if params.use_ghm:
                ghm_weights = calc_ghm_weights(
                    np.array(model_logit_val), np.array(data['gt_classes']))
            feed_dict = {
                network.ghm_weights: ghm_weights,
            }
            fetches = [model_loss, regularization_loss,
                       total_loss, summary_op, train_dummy]
            ...
            if iter % params.validation_step == 0 and len(data_loader.validation_docs):
                ...
                with open('validation_statistics.txt', 'a') as f:
                    for a in [iter, idx*params.validation_step, recall, acc_strict, acc_soft,
                              validation_recall[idx], validation_acc_strict[idx], validation_acc_soft[idx]]:
                        f.write(str(a))
                        f.write(',')
                    f.write('\n')
                ...
                if iter >= params.ckpt_save_step and validation_acc_strict[-1] > max(validation_acc_strict[:-1]+[0]):
                    ...
                    save_ckpt(sess, params.ckpt_path, params.save_prefix,
                              data_loader, network, num_words, num_classes, iter+1)
                    print(
